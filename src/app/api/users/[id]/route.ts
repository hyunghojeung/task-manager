...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session || (session.user.role !== "admin" && session.user.role !== "super_admin")) return unauthorized();
  const { id } = await params;
  const body = await request.json();
  const supabase = getSupabase();

  // 자기 자신을 user로 강등 방지 (admin이나 super_admin 유지)
  if (body.role === "user" && id === session.user.id) {
    return NextResponse.json({ error: "자기 자신의 관리자 권한은 변경할 수 없습니다." }, { status: 400 });
  }

  // 회사의 마지막 admin/super_admin을 user로 변경 방지
  if (body.role === "user") {
    const { count } = await supabase.from("users").select("*", { count: "exact", head: true }).eq("company_id", session.company.id).in("role", ["admin", "super_admin"]);
    if ((count || 0) <= 1) {
